...
function download(torrent, dest, option, callbackProgress = () => {}){
  ...
  const options = {
    timeout: isIntegerPositiveOrZero(option.timeout) ? option.timeout : 10,
    exclusion: isArrayOfStringNotEmpty(option.exclusion) ? option.exclusion : [],
    downloadLimit: isIntegerPositive(option.downloadLimit) ? option.downloadLimit : -1,
    uploadLimit: isIntegerPositive(option.uploadLimit) ? option.uploadLimit : 100 
  };
